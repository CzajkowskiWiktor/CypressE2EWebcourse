...
export class PopoverPage{
    ...
    checkTemplateWithCard(){
        cy.contains('nb-card', 'Template Popovers').then(card =>
        {
            ...
            cy.get('nb-popover').find('nb-card').then(popoverForm =>
            {
                cy.wrap(popoverForm).find('nb-card-header').should('contain', 'Hello!')
                cy.wrap(popoverForm).find('nb-card-body').should('contain', 'Far far away, behind the word mountains, far from the countries Vokalia and Consonantia, there live the blind texts. Separated they live in Bookmarksgrove right at the coast of the Semantics, a large language ocean')
            })
        })
    }
    ...
    checkAllComponentPopovers(){
        cy.contains('nb-card', 'Component Popovers').then(card =>
            {
                //check title
                cy.wrap(card).find('nb-card-header').should('contain', 'Component Popovers')
                //check body text
                cy.wrap(card).find('nb-card-body').find('p').should('contain', 'Same way popover can render any angular compnoent.')
                //find button with tabs
                cy.wrap(card).contains('With tabs').click()
                //form popover verify content
                cy.get('nb-popover').then(popoverForm =>
