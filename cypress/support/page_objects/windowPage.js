export class WindowPage {
  fillWindowFormMinimizeExpandVerify(subject, text) {
    ...
    cy.contains("nb-card", "Window Form")
      .contains("button", "Open window form")
      .click();
  ...
  checkWindowTemplate() {
    //get button
    cy.contains("nb-card", "Window Form")
      .contains("button", "Open window with")
      .click();
    ...
    cy.get("nb-window.full-screen").then((window) => {
      ...
      cy.wrap(window)
        .find("nb-card-header")
        .find(".title")
        .should("contain", "Window content from template");
      //check window text
      cy.wrap(window)
        .find("nb-card-body")
        .find("p")
  ...
  openWindowWithBackdrop() {
    //get button
    cy.contains("nb-card", "Window Without")
      .contains("button", "Open window with")
      .click();
    //check the form template
    cy.get("nb-window.full-screen").then((window) => {
  ...
  openWindowWithoutBackdrop() {
    //get button
    cy.contains("nb-card", "Window Without")
      .contains("button", "Open window without")
      .click();
    //check the form template
    cy.get("nb-window.full-screen").then((window) => {
      //verify window title
      cy.wrap(window)
        .find("nb-card-header")
        .find(".title")
        .should("contain", "Window without backdrop");
      //check window text
      cy.wrap(window)
        .find("nb-card-body")
        .should("contain", "Disabled close on escape click.");
      //2#closing the window by esc btn
      cy.wrap(window).find("nb-card-header").find(".title").type("{esc}");
